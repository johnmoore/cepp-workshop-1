...
class HTTPrequest {
	...
	function sendGET($url, $cookie="") {
		...
		$ch = curl_init($parts['scheme']."://".$parts['host']);
		...
		$this->cookie = $this->parseCookie($this->response['header'].$this->response['data']);
	}
	...
	/* "Follows" a 302 response
	*/
	function follow($cookie = true) {
		while (strpos($this->response['header'].$this->response['data'], "Location: ")) {
			$loc = reset(explode("\r\n", end(explode("Location: ", $this->response['header'].$this->response['data']))));
			$parts = parse_url($loc);
			$this->sendGET($loc, ($cookie ? $this->cookie : ""));
		}
	}
